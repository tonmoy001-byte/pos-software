        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-surface w-full max-w-2xl rounded-[2.5rem] p-10 card-shadow space-y-8 relative animate-in zoom-in-95 duration-200">
            <button 
              ...
              className="absolute top-8 right-8 text-secondary hover:text-foreground transition-colors"
            >
              <X className="w-8 h-8" />
            </button>
                  ...
                  {isAddingCustomer ? (
                    <div className="space-y-3 p-5 bg-background rounded-3xl border-2 border-primary/10 animate-in slide-in-from-top-2">
                      <input 
                        ...
                        className="w-full bg-transparent border-b border-border py-2 text-sm outline-none focus:border-primary font-bold"
                      />
                      <input 
                      ...
                      <button 
                        ...
                        className="w-full py-3 bg-primary/10 text-primary rounded-xl text-xs font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all"
                      >
                        Create & Select
                      </button>
                    </div>
                      ...
                      {selectedCustomer ? (
                        ...
                        <>
                          ...
                          <input 
                            ...
                            className="w-full pl-12 pr-4 py-4 rounded-2xl bg-background border-2 border-border focus:border-primary outline-none transition-all font-bold text-sm"
                          />
                            ...
                            <div className="absolute top-full left-0 right-0 bg-surface border-2 border-border rounded-2xl mt-2 overflow-hidden z-10 shadow-xl">
                              {customerResults.map(c => (
                                <button 
                                  ...
                                  className="w-full p-4 text-left hover:bg-primary/5 transition-colors border-b border-border last:border-0 flex items-center justify-between"
                                >
                                  <div>
      ...
      {lastSale && (
        <ThermalReceipt
          data={{
            ...
            due: lastSale.dueAmount,
            paymentMethod: lastSale.payments?.[0]?.method || "CASH",
            store: {
              name: invoiceConfig?.general?.businessName || currentStore?.name || "Store",
              address: invoiceConfig?.general?.address || currentStore?.address || "",
              phone: invoiceConfig?.general?.phone || currentStore?.phone || "",
            },
          }}
      ...
      <div className="flex-1 flex flex-col p-6 space-y-6 overflow-hidden">
        <div className="flex gap-4">
          ...
          <button 
            ...
            className="bg-primary/10 text-primary p-4 rounded-2xl border border-primary/20 hover:bg-primary/20 transition-colors"
      ...
      <div className="w-[500px] bg-surface border-l border-border flex flex-col card-shadow">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex p-1 bg-background rounded-xl border border-border flex-1">
                <button 
                  onClick={() => { setCustomerType("WALKING"); setSelectedCustomer(null); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-bold rounded-lg transition-all ${customerType === "WALKING" ? "bg-white text-primary shadow-sm" : "text-secondary"}`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-bold rounded-lg transition-all ${customerType === "REGISTERED" ? "bg-white text-primary shadow-sm" : "text-secondary"}`}
                >
              ...
              <button 
                onClick={() => { setIsAddingCustomer(true); setCustomerType("REGISTERED"); }}
                className="p-2 bg-primary/10 text-primary rounded-xl hover:bg-primary/20 transition-colors ml-2"
        ...
        <div className="p-6 bg-background border-t border-border space-y-4">
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-3 py-5 bg-primary text-white rounded-2xl font-black text-lg shadow-xl shadow-primary/30 hover:scale-[1.02] transition-all group disabled:opacity-50"
          >
