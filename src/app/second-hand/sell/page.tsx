...
export default function SellUsedPhonePage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        const all = Array.isArray(json) ? json : ((json as any).products || []);
        setProducts(all.filter((p: any) => p.category === "Second Hand" && (p.stock ?? 0) > 0));
      } catch (err) {
  ...
  const handleCloseReceipt = () => {
    ...
    safeFetch("/api/products").then(data => {
      const all = Array.isArray(data) ? data : ((data as any).products || []);
      setProducts(all.filter((p: any) => p.category === "Second Hand" && (p.stock ?? 0) > 0));
    }).catch(() => {});
  };
  ...
  if (loading) return <div className="p-8 animate-pulse text-secondary font-bold">Loading Second-Hand Inventory...</div>;
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-surface w-full max-w-2xl rounded-[2.5rem] p-10 card-shadow space-y-8 relative animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <h2 className="text-3xl font-black tracking-tight">Complete Sale</h2>
                <p className="text-secondary font-medium">Finalize second-hand sale</p>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-black text-primary uppercase tracking-widest">Second-Hand Sales</span>
            </div>
            <h1 className="text-2xl font-bold text-foreground">Sell Used Phone</h1>
            <p className="text-xs text-secondary">Sell items from your used phone inventory</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredProducts.map((product) => (
              <div 
                ...
                className="bg-surface p-4 rounded-2xl border border-green-300 cursor-pointer group transition-all"
              >
                <div className="aspect-square bg-background rounded-xl mb-4 flex items-center justify-center relative overflow-hidden">
                  {product.metadata?.imageUrl ? (
                    ...
                    <Smartphone className="w-12 h-12 text-green-400 group-hover:scale-110 transition-transform" />
                  )}
                  <span className="absolute top-2 right-2 text-[10px] font-bold px-2 py-1 rounded-md bg-green-500 text-white">
                    IN STOCK
                  </span>
                  {(product.stock ?? 0) > 0 && (
                ...
                {product.cost > 0 && (
                  <p className="text-[10px] text-secondary mt-1">
                    Cost: {formatCurrency(product.cost)} → Margin: {formatCurrency(product.price - product.cost)} ({Math.round(((product.price - product.cost) / product.cost) * 100)}%)
                  </p>
                )}
                <div className="flex justify-between items-center mt-3 pt-3 border-t border-border/50">
                  <span className="font-black text-primary">{formatCurrency(product.price)}</span>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center transition-all bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white">
            ...
            {filteredProducts.length === 0 && (
              <p className="text-secondary italic col-span-full text-center py-8">No second-hand products in inventory.</p>
            )}
          </div>
        </div>
      </div>
